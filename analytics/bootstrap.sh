...
do
  kibana_status=$(curl -I -m2 $kibana_base_url/app/kibana 2>>.bootstrap.log | head -n 1 | cut -d$' ' -f2)  
  if [ "$kibana_status" != "$kibana_status_desired" ]
  then
    echo "$kibana_status" >>.bootstrap.log
    sleep 1
  fi
  bootstrap_progress
done
...
curl $kibana_base_url/api/saved_objects/index-pattern/1208b8f0-815b-11ea-b0b2-c9a8a88fbfb2?overwrite=true -s \
  ...
  -H 'kbn-xsrf: true' \
  -d @analytics/data/kibana/index-patterns/tyk-analytics.json >>.bootstrap.log
bootstrap_progress
...
curl $kibana_base_url/api/saved_objects/visualization/407e91c0-8168-11ea-9323-293461ad91e5?overwrite=true -s \
  -H 'Content-Type: application/json' \
  -H 'kbn-xsrf: true' \
  -d @analytics/data/kibana/visualizations/request-count-by-time.json >>.bootstrap.log
bootstrap_progress
...
docker-compose stop tyk-pump 2> .bootstrap.log
bootstrap_progress
...
curl -s localhost:8080/basic-open-api/get >> .bootstrap.log
