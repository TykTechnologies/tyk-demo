...
| INSTRUMENTATION_ENABLED | Controls whether the instrumentation feature is enabled or disabled | No | `0` - Set automatically by the `up.sh` script |
| TRACING_ENABLED | Controls whether the tracing feature is enabled or disabled | No | `false` - Set automatically by the `up.sh` script |
| GATEWAY_VERSION | Sets the Tyk Gateway container image tag | No | Based on the latest release |
| GATEWAY_LOGLEVEL | Sets the log level for the Tyk Gateway application | No | `info` |
