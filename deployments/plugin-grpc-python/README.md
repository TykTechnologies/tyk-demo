...
To bootstrap this deployment, run the `up.sh` command with the `plugin-grpc-python` argument:

```shell
./up.sh plugin-grpc-python
``` 
...
```shell
./deployments/plugin-grpc-python/scripts/hmac.sh
```
