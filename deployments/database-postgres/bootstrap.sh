...
eval $(generate_docker_compose_command) exec -T -u postgres tyk-postgres sh -c \"psql -U postgres -c \'CREATE DATABASE tyk_analytics\'\" 1>>logs/bootstrap.log 2>&1
if [ "$?" != 0 ]; then
...
eval $(generate_docker_compose_command) exec -T tyk-dashboard sh -c \"/opt/tyk-dashboard/tyk-analytics migrate-sql\" 1>>logs/bootstrap.log 2>&1
if [ "$?" != 0 ]; then
...
eval $(generate_docker_compose_command) rm -s -f tyk-dashboard 1>>logs/bootstrap.log 2>&1
if [ "$?" != 0 ]; then
...
eval $(generate_docker_compose_command) run -T -d -e TYK_DB_MONGOURL='' --service-ports --use-aliases tyk-dashboard 1>>logs/bootstrap.log 2>&1
if [ "$?" != 0 ]; then
