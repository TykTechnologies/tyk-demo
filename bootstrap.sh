...
echo "Begin bootstrap.sh" >.bootstrap.log
...
echo "Prevent instrumentation being enabled without the instrumentation service being available" >>.bootstrap.log
...
then
  # replace (done this way to be compatible across different linux versions)
  sed -i.bak 's/'"$instrumentation_setting_enabled"'/'"$instrumentation_setting_disabled"'/g' ./.env
  rm .env.bak
  docker-compose up --force-recreate -d >>.bootstrap.log
fi
bootstrap_progress

echo "Prevent tracing being enabled without the tracing service being available" >>.bootstrap.log
...
then
  ...
  rm .env.bak
  docker-compose restart >>.bootstrap.log
fi
bootstrap_progress
